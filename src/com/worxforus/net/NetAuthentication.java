...
/**
 * This class is used to contain the login credentials in memory and to handle
 * actual responses to the network login attempts.
 * 
...
public class NetAuthentication {
	...
	private String accessToken = ""; // accessToken can be used instead of a
										// password
	private String uuid = ""; // when combined with accessToken it can be used
								// instead of a password
	private volatile boolean isLoggedIn = false; // store if we have been
													// authenticated
	private long lastLoginTime = 0; // stores last time we authenticated
	private int usernum = -1;

	protected volatile int loginStatus = NOT_SET; // starts as false to indicate
													// we haven't attempted the
													// connection yet

	public static final int NO_ERRORS = 0;
	public static final int NOT_SET = -1; // login information has not been
											// checked
	public static final int LOGIN_FAILURE = 1; // login supplied but not valid
	public static final int NETWORK_ERROR = 2; // could not communicate to
												// server
	public static final int SERVER_ERROR = 3; // server response could not be
												// parsed
	public static final int NOT_LOGGED_IN = 4; // server indicates user is not
												// logged in - should only
												// happen in context other than
												// trying to log in.

	public static long LOGIN_REFRESH_TIME_SECONDS = 28800; // retries login
															// after 8 hours if
															// requested
	...
	public static String getLoginStatusMessage(String appName) {
		...
		} else if (NetAuthentication.getInstance().loginStatus == NOT_SET) {
			message = "Not logged in.  Please configure or create your account.";
		}
	...
	public static String getTokenStatusMessage() {
		String message = "Waiting to complete registration.";
		if (NetAuthentication.getInstance().accessToken.length() > 0) {
			message = "Token received.";
		}
		if (NetAuthentication.isUsernameSet()) {
			message = "Token not needed - using username/password for login.";
		}
		return message;
	}
	...
	/**
	 * Returns the last authenticated user - does not reset on invalidate (ie.
	...
	/**
	 * Use this function after calling loadUsername and loadPassword Run inside
	 * a separate thread using Runnable or ASyncTask since the network calls may
	 * block for a while
	 * 
	 * The NetHelper interface is used to attempt access multiple retries if
	 * needed. Use NetAuthentication.getLoginStatus() to verify success or
	 * failure information
	 * 
	 * If successful, this function calls the NetAuthenticationHelper to persist
	...
	public static NetResult authenticate(String host, NetAuthenticationHelper authHelper) {
		synchronized (NetAuthentication.getInstance()) { // so we can only try
															// to authenticate
															// one at a time
			...
			if (NetAuthentication.isUsernameSet()) {
				...
				result.net_success = true; // since this is not a network
											// problem
			...
			// this function below should be called in
			// NetAUthenticationHelper.handleXXXLogin(...);
			// NetHandler.handleGenericJsonResponseHelper(result,
			// NetAuthentication.class.getName());
			...
			Utils.LogD(NetAuthentication.class.getName(), "User authentication was "
					+ NetAuthentication.getInstance().getLoginStatusMessage("Server"));
	...
	/**
	 * Returns true if the cache value looks ok and we think we are still logged
	 * in. Returns false if not.
	 * 
	 * @return
	 */
	public static boolean isCurrentAuthenticationValid() {
		if (NetAuthentication.getInstance().isLoggedIn) {
			...
			long time_diff_sec = time_diff_nano / 1000000000; // convert
																// nanoseconds
																// to seconds:
																// nano/micro/milli
	...
	/**
	 * Call this function whenever the login information changes or login can be
	 * re-queried. This clears the login cache. You should also consider
	 * clearing the credential cache with AuthNetHandler.reset();
	 */
	...
	/**
	 * Clears the login information and credentials stored in memory along with
	 * restting the login cache. You should also consider clearing the
	 * credential cache with AuthNetHandler.reset();
	 */
	public static void wipeCredentials() {
		NetAuthentication.getInstance().username = "";
		NetAuthentication.getInstance().password = "";
		NetAuthentication.getInstance().accessToken = "";
		NetAuthentication.reset();
	}

	/**
	 * Mark logged in cache as invalid - so it will be rechecked next time
	...
	/**
	 * This function returns true if there is a username present that allows for
	 * the system to attempt a login.
	 * 
	 * @return
	 */
	...
	/**
	 * This function returns true if there is a accessToken (or other token)
	 * present that allows for the system to attempt a login.
	 * 
	 * @return
	 */
	public static boolean isTokenSet() {
		if ((NetAuthentication.getInstance().accessToken != null && NetAuthentication.getInstance().accessToken
				.length() > 0)) {
			return true;
		} else {
			return false;
	...
	/**
	 * This function returns true if there is a username or accessToken (or
	 * other token) present that allows for the system to attempt a login.
	 * 
	 * @return
	 */
	...
	/**
	 * NetAuthenticationHelper defines the necessary functions needed for login
	 * handling.
	 * 
	 * @author sbossen
	 * 
	...
	public interface NetAuthenticationHelper {
		...
		/**
		 * Use this function to save a usernumber into the application. Store in
		 * the preferences for example. This is called on a successful call to
		 * NetAuthentication.authenticate(...)
		 * 
		 * @param result
		 * @return
		 */
		...
		/**
		 * Use this function to read a given response from a website and return
		 * the user number
		 * 
		 * @param result
		 * @return
		 ...
		 * This simulates a login failure so it can be reported back to the
		 * calling class
		 * 
		 * @param result
		 */
		...
		/**
		 * Use this function to force the result to appear as a login success
		 * due to cache hit This simulates a login success so it can be reported
		...
		/**
		 * With this function, AuthNetHandler can use it to send an error
		 * message back up to the application
		 * 
		 * @return
		 */
		 ...
		 * 
		 * @param netResult
		 *            - most useful for netResult.object to be a json object
		 * @return int containing loginStatus based on given netResult
		 */
		public int validateLoginResponse(NetResult netResult);

		/**
		 * This function is to be used by external classes that check to see if
		 * a login error was created. Run after using a network command to any
		 * page other than the login interface page. This does not check for all
		 * login failures, but only: NO_ERRORS NOT_LOGGED_IN
		 * 
		 * If this returns the error with the NOT_LOGGED_IN string, then
		 * NetAuthentication.invalidate() should be called
		 * 
		 * @param netResult
		 *            - most useful for netResult.object to be a json object
		 * @return int containing loginStatus based on given netResult - either
		 *         NO_ERRORS or NOT_LOGGED_IN
		 */
		public int peekForNotLoggedInError(NetResult netResult);

		/**
		 * Login to the getLoginURL() with the given username and password.
		 * Expect a json success response or failure makes no determination of
		 * success or failure of login, just network connection run
		 * checkForLoginError(...) to determine type of login failure
		 * 
		 * @param username
		 * @param password
		 * @return
